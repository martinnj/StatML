...
ourAccTrain    = 1 - (nnz(pred_train - y_train) / length(y_train))
ourAccTest     = 1 - (nnz(pred_test - y_test) / length(y_test))

linclass = ClassificationDiscriminant.fit(X_train,y_train);
pred_train = predict(linclass,X_train);
pred_test = predict(linclass,X_test);

theirAccTrain = 1 - (nnz(pred_train - y_train) / length(y_train))
theirAccTest = 1 - (nnz(pred_test - y_test) / length(y_test))